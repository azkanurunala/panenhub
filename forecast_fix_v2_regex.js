...
const replacement = `else if (name === 'Lihat Detail Forecast') {
  ...
  else if (name === 'Pesan Sekarang') {`; // Included next block signature just to be completely safe during regex replace.

const file = 'c:\\Prospects\\panenhub\\v2.html';
const content = fs.readFileSync(file, 'utf8');

// Use Regex to match from Lihat Detail Forecast to Pesan Sekarang
const regex = /else if \(name === 'Lihat Detail Forecast'\) \{[\s\S]*?else if \(name === 'Pesan Sekarang'\) \{/g;
if (regex.test(content)) {
  fs.writeFileSync(file, content.replace(regex, replacement), 'utf8');
  console.log('Fixed v2.html using regex constraint');
} else {
